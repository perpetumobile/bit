...
//fake CliMain
//connection will be managed and provided to CliClient
class CliMain {
	...
	static void updateCompletor(Set<String> set) {
	}
}
...
public class CliClient
{
    ...
    public static String CLI_CLIENT_CQL3_ENABLE_CONFIG_KEY = "CliClient.CQL3.Enable";
    public static boolean CLI_CLIENT_CQL3_ENABLE_CONFIG_DEFAULT = true;
    
    private boolean cql3Enable = CLI_CLIENT_CQL3_ENABLE_CONFIG_DEFAULT;
    ...
    public CliClient(CliSessionState cliSessionState, Cassandra.Client thriftClient)
    {
        ...
        this.cql3Enable = Config.getInstance().getBooleanProperty(CLI_CLIENT_CQL3_ENABLE_CONFIG_KEY, CLI_CLIENT_CQL3_ENABLE_CONFIG_DEFAULT);
    }
    ...
    public List<KeySlice> executeCLIStatement(String statement) throws CharacterCodingException, TException, TimedOutException, NotFoundException, NoSuchFieldException, InvalidRequestException, UnavailableException, InstantiationException, IllegalAccessException, ClassNotFoundException
    {
        ...
        try
        {
            switch (tree.getType())
            {
                ...
                case CliParser.NODE_THRIFT_GET:
                    result = executeGet(tree);
                    break;
                case CliParser.NODE_THRIFT_GET_WITH_CONDITIONS:
                    result = executeGetWithConditions(tree);
                    break;
            ...
            throws NotFoundException, InvalidRequestException, TException
    {
        ...
        if (!(keyspacesMap.containsKey(keyspace)))
        {
            ...
            if(cql3Enable) {
            	...
            	cql3KeyspacesMap.put(keyspace, new HashMap<String, CfDef>());
            }
    ...
    private List<KeySlice> doSlice(String keyspace, ByteBuffer key, String columnFamily, byte[] superColumnName, ByteBuffer columnName, int limit)
            throws InvalidRequestException, UnavailableException, TimedOutException, TException, IllegalAccessException, NotFoundException, InstantiationException, NoSuchFieldException
    {
        ...
        if(columnName != null) {
        	sp.addToColumn_names(columnName);
        	sp.setSlice_range(null);
        } else {
        	sp.setColumn_names(null);
        	sp.setSlice_range(range);
        }
        List<ColumnOrSuperColumn> columns = thriftClient.get_slice(key, parent, sp, consistencyLevel);

        KeySlice keySlice = new KeySlice();
        keySlice.setKey(key);
        keySlice.setColumns(columns);
        
        ArrayList<KeySlice> result = new ArrayList<KeySlice>();
        result.add(keySlice);
        elapsedTime(startTime);
        return result;
    ...
    private List<KeySlice>  executeGet(Tree statement)
            throws TException, NotFoundException, InvalidRequestException, UnavailableException, TimedOutException, IllegalAccessException, InstantiationException, ClassNotFoundException, NoSuchFieldException
    {
        ...
        if (columnSpecCnt == 0)
        {
            return doSlice(keySpace, key, columnFamily, superColumnName, null, limit);
        }
        ...
        else if (columnSpecCnt == 1)
        {
            ...
            if (isSuper)
            {
                ...
                return doSlice(keySpace, key, columnFamily, superColumnName, columnName, limit);
            }
        }
     ...
     */
    private List<KeySlice> executeGetWithConditions(Tree statement)
    {
        if (!CliMain.isConnected() || !hasKeySpace())
            return null;
    ...
    private List<KeySlice> executeList(Tree statement)
